...
module.exports = async function ({
  ...
  await assert('gamepad reconnects after page reload', async () => {
  ...
  await assert('gamepad identity is correct after reload', async () => {
  ...
  await assert('key bindings work after page reload', async () => {
  ...
  await assert('axis bindings work after page reload', async () => {
  ...
  await assert('only one gamepad exists after multiple reloads', async () => {
    for (let i = 0; i < 3; i++) {
      await page.reload({ waitUntil: 'load' });
      await waitForStatus(page, 'connected', 15000);
    }
  ...
  await assert('gamepad is at index 0 after multiple reloads', async () => {
    const identity = await getGamepadIdentity(page);
    expect(identity.index).toBe(0);
  });
  ...
  await assert('gamepadconnected event fires after reload', async () => {
    await page.reload({ waitUntil: 'load' });
    await waitForStatus(page, 'connected', 15000);
  ...
  await assert('no phantom input after reload', async () => {
  ...
  await assert('gamepad works after navigating away and back', async () => {
