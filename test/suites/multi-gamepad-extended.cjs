...
module.exports = async function ({
  ...
  try {
    ...
    await assert(
      'physical pad appears at slot 1 (not virtual slot 0)',
      async () => {
    ...
    await assert(
      'physical pad button press visible at assigned slot 1',
      async () => {
        physicalPad.pressButton(BTN_SOUTH);
        await waitForPadButton(page, 1, 0, true);
        physicalPad.releaseButton(BTN_SOUTH);
    ...
    await assert(
      'virtual pad slot 0 unaffected by physical pad input',
      async () => {
    ...
    await assert(
      'physical pad disconnect fires gamepaddisconnected',
      async () => {
  ...
  try {
    ...
    await assert(
      'combine mode: physical button at virtual slot 0 merges with virtual pad',
      async () => {
    ...
    await assert(
      'combine mode: virtual key press at slot 0 still works',
      async () => {
        await page.keyboard.down('z');
        await waitForPadButton(page, 0, 0, true);
        await page.keyboard.up('z');
    ...
    await assert(
      'combine mode: physical and virtual inputs OR together at slot 0',
      async () => {
    ...
    await assert(
      'combine mode: physical pad at virtual slot does not appear at slot 1',
      async () => {
